...
''' v8: Inputs base frequency and subsequent frequencies. Draws a perturbation with the width which is taken
...
def get_initial_coordinates(a, scale):
...
def draw_perturbation(w, x00, y00, x01, y01, scale, a, height, delta):
    global dirname
    h = height * (1 - 1/scale)
    filename = dirname + '\Element_perturbation.txt'
    f = open(filename, 'w')
    ...
    x_05 = x00 - h/sqrt(3)
    y_05 = y00 - h
    x_06 = x_02 - (h - delta)/sqrt(3)
    y_06 = y00 - h + delta
    x_07 = x_03 + (h - delta) / sqrt(3)
    y_07 = y_06
    x_08 = x_04 + h / sqrt(3)
    y_08 = y_05
    height /= scale
    s = '{0}, {1}\n'.format(x_01 - a/2, y_01 - height/3) + '{0}, {1}\n'.format(x_02 - a/2, y_02 - height/3) + \
        '{0}, {1}\n'.format(x_06 - a/2, y_06 - height/3) + '{0}, {1}\n'.format(x_07 - a/2, y_07 - height/3) +\
        '{0}, {1}\n'.format(x_03 - a/2, y_03 - height/3) + '{0}, {1}\n'.format(x_04 - a/2, y_04 - height/3) +\
        '{0}, {1}\n'.format(x_08 - a/2, y_08 - height/3) + '{0}, {1}\n'.format(x_05 - a/2, y_05 - height/3)
    s += '{0}, {1}'.format(x_01 - a/2, y_01 - height/3)
    print s + '\n'
    f.write(s + '\n')
    f.close()


def calculate_coordinates(canvas, iteration, x00, y00, x01, y01, x02, y02, n1, n2, a, h, delta):
    a /= (n1 + 1)
    draw_perturbation(w, x00, y00, x02, y02, n1+1, a, h, delta)
    h /= (n1+1)
    draw_helper(canvas, iteration, x00, y00, x01, y01, x02, y02, n1, n2, a, h, True)
...
delta = float(input('Width of Triangular Ring in mm (Recommended : 0.25 mm): '))
assert(delta >= 0), "Width of Triangular Ring should be Positive"

side = calculate_side(substrate_thickness)
height = sqrt(3) * side / 2


get_initial_coordinates(side, n[0])
...
calculate_coordinates(w, itr+1, x00, y00, x01, y01, x02, y02, n[0]-1, n[1]-1, side, height, delta)
